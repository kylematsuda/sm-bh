...
ts = np.linspace(0, (N+1)*delta, num=(N+1))
for i in range(0, L):
	ax[i].plot(ts, np.absolute(n_avg[i,:]), 'bo', label="TEBD site {0}".format(i))
	ax[i].legend()
	ax[i].set_yticks(np.linspace(0,6,num=2))
	ax[i].set_yticklabels(np.linspace(0,6,num=2))
	ax[i].set_ylim([-0.1, 4])
ax[0].set_title(r"TEBD simulation: $L = {0}$, $d = {1}$, $\chi = {2}$".format(L,d,chi), fontsize=18)
f.subplots_adjust(hspace=0)
plt.setp([a.get_xticklabels() for a in f.axes[:-1]], visible=False)
