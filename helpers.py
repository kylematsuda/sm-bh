...
class TEBD(object):
	...
	def __init__(self, model, sim, init, logs):
		...
		self.tensorA[L-1] = np.transpose(np.tensordot(np.diag(self.Lambda[L-2]), self.tensorA[L-1], axes=(0,-1)), (1,0))
		self.tau = 0
		self.model = model
		self.sim = sim
		self.init = init
	...
	def Update(self, V, l):
		...
		# The Gamma_0 tensor has one less index... need to treat slightly differently
			...
			A_l = np.zeros((d, chi), dtype=np.complex64)
			A_l[:, 0:d] = A
			self.tensorA[l] = A_l

			A_l_conj = np.conjugate(A_l)
			Theta = np.reshape(Theta, (d, d, chi))
			A_lp1 = np.transpose(np.tensordot(A_l_conj, Theta, axes=(0,0)))
			...
			# A_l_conj = np.conjugate(A_l)
			# Theta = np.reshape(Theta, (d, d, chi))
			# A_lp1 = np.transpose(np.tensordot(A_l_conj, Theta, axes=([0,1],[0,2])))
			# self.tensorA[l+1] = A_lp1
			self.tensorA[l+1][:,0:d] = np.transpose(np.tensordot(np.diag(B), C, axes=(0,0)))
	...
	def Single_Site_Rho(self, k):
		...
		if (k != 0 and k != L - 1):
			...
			Rho = np.tensordot(np.conjugate(A_k), A_k, axes=(-1, -1))
			Rho = np.transpose(Rho)
		return Rho
